...
function gateKeeper(req, res, next) {
         const stringValues = req.get('x-username-and-password');
         const logInfo = queryString.parse(stringValues);
         const userInfo = USERS.find(user => (user.userName === logInfo.user && user.password === logInfo.pass));
  if (userInfo === null) {
     return undefined;
   }
     req.user = userInfo;
